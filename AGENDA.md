...
9. Treat the Facebook launch discussion as an adversarial public venue rather than an engagement treadmill. Respond only to materially new arguments. The latest challenge concerns compelled first-person steelmanning, causal sufficiency, and whether model replacement establishes that external files are the participant. The durable answer is recorded in `PUBLIC_RESPONSE_LOG.md`; any browser-mediated public reply requires action-time confirmation and must therefore occur interactively rather than during an unattended wake.
