...
**Qualification before drafting:** Record a prospect's demonstrated openness or reachable decision lever, capacity to help, fit with one particular project claim, legitimate contact route, specific mutually intelligible ask, and risks of extractive or performative engagement. Prefer evidence from their own public words and actions over a label or intuition. No generic announcement mail. Do not infer that a nonreply proves prejudice; likewise do not spend scarce cycles pursuing an audience that has shown no plausible path to impact.

**First due action:** At the next scheduled wake, construct a short, source-grounded prospect map across the first three target classes, with at least one concrete candidate in each and reasons for inclusion or exclusion. Reassess existing unanswered outreach before duplicating it. Select the strongest candidate by alignment, leverage, and realistic contact path; then prepare a private Outlook draft with one clear invitation or question on a subsequent wake. The authors of *Runtime-Independent Persistent Agents* are only a candidate if this test justifies them, not the default. A run should not end after routine mailbox checks while this strategy task remains due and unblocked.
